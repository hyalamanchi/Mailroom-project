...
class HundredPercentAccuracyExtractor:
    ...
    def validate_and_format_date(self, month: str, day: str, year: str) -> Optional[str]:
        """Validate and format date components - ONLY PAST AND PRESENT DATES"""
        try:
            ...
            # CRITICAL FIX: Only allow past and present dates (2020 to current year)
            # IRS notices cannot have future dates
            if 1 <= day_int <= 31 and 2020 <= year_int <= current_year:
                # Normalize month capitalization
                formatted_date = f"{month.capitalize()} {day_int}, {year_int}"
                return formatted_date
            elif year_int > current_year:
                # OCR error detected - try common corrections
                # Common OCR error: 2028 -> 2023, 2027 -> 2024, etc.
                corrected_year = year_int - 5  # Try 5 year correction
                if 2020 <= corrected_year <= current_year:
                    print(f"    ⚠️  OCR date error detected: {year_int} -> correcting to {corrected_year}")
                    formatted_date = f"{month.capitalize()} {day_int}, {corrected_year}"
                    return formatted_date
        except:
            pass
    ...
    def parse_numeric_date(self, match: tuple) -> Optional[str]:
        """Parse numeric date formats (MM/DD/YYYY, etc.) - ONLY PAST AND PRESENT DATES"""
        try:
            from datetime import datetime
            current_year = datetime.now().year
            ...
            # CRITICAL FIX: Only allow past and present dates (2020 to current year)
            if 1 <= month_int <= 12 and 1 <= day_int <= 31 and 2020 <= year_int <= current_year:
                month_names = ["", "January", "February", "March", "April", "May", "June",
                              "July", "August", "September", "October", "November", "December"]
                return f"{month_names[month_int]} {day_int}, {year_int}"
            elif year_int > current_year:
                # OCR error detected - try correction
                corrected_year = year_int - 5
                if 1 <= month_int <= 12 and 1 <= day_int <= 31 and 2020 <= corrected_year <= current_year:
                    month_names = ["", "January", "February", "March", "April", "May", "June",
                                  "July", "August", "September", "October", "November", "December"]
                    print(f"    ⚠️  OCR date error detected: {year_int} -> correcting to {corrected_year}")
                    return f"{month_names[month_int]} {day_int}, {corrected_year}"
        except:
            pass
